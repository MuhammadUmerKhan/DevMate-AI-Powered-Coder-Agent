...
class File(BaseModel):
    ...
    purpose: str = Field(
                    description="The purpose of the file, e.g. 'main application logic', 'user interface', 'database schema', etc."
                    )

class Plan(BaseModel):
    name: str = Field(description="The name of app to built.")
    ...
    features: list[str] = Field(description="A list of features to be implemented in the app, e.g. 'authentication', 'user management', 'database', etc.")
...
class TaskPlan(BaseModel):
    ...
    model_config = ConfigDict(extra="forbid")  # Changed from extra="allow" to extra="forbid" to set additionalProperties=False
